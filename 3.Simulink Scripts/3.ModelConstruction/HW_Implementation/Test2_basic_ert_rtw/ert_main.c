...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Test2_basic_M, 0);
  ...
  Test2_basic_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(Test2_basic_M->extModeInfo, &rtmGetTFinal
    (Test2_basic_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Test2_basic_M, true);
    }
  }
